                ...
                <tr>
                    ...
                    <td>
                        ...
                        <form method="POST" action="{{ url('/product' . '/' . $item->id) }}" accept-charset="UTF-8" style="display:inline">
                            {{ method_field('DELETE') }}
                            @csrf
                            <button type="submit" class="btn btn-danger btn-sm" title="Apagar Produto" onclick="return confirm(&quot;Confirm delete?&quot;)">Apagar</button>
                        </form>
                    </td>
                </tr>
            @endforeach
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <form method="POST" action="{{ url('/product-composite' . '/' . $item->id) }}" accept-charset="UTF-8" style="display:inline">
                            {{ method_field('DELETE') }}
                            @csrf
                            <button type="submit" class="btn btn-danger btn-sm" title="Apagar Produto" onclick="return confirm(&quot;Confirma exclusão?&quot;)"></i>Apagar</button>
                        </form>
                    </td>
                </tr>
            @endforeach
